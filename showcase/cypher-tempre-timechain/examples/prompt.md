...
Run the Showcase-owned `cypher-tempre-self-model` skill against a new disposable
chain. Seal exactly one synthetic public ring, derive its public keyless CPHY
observation address, query the canonical CPHY contract through the skill's
allowlisted read-only Base RPC path, and verify both the CPHY ledger and the
Timechain.
