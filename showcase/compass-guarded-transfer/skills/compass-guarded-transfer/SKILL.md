...
2. Install and authenticate the trusted `acp` command separately. This showcase targets the documented ACP CLI `v1.0.24` command/output shape: `acp wallet sol address --cluster devnet --json` returns `{ "address": "<solana-address>" }`, and transfer returns `{ "signature": "<solana-signature>" }`. Tests use mocks; no live CLI proof is claimed.
